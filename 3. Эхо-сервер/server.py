import json # модуль для работы с файлами формата json (текстовый формат для хранения и обмена данными)
import random # модуль для работы с рандомными значениями
...
import hashlib # модуль для шифрования строк
...
class ClientThread(Thread):
    ...
    def login(self): # вход для пользователей
        self.send_msg('Enter username')
    ...
    def success_login(self): # успешный вход
        self.send_msg(f'Login is successful')
        save_users()

    def close_connection(self, reason=''): # завершение подключения
        logging.info(f'Connection is closed {self.addr} {" - " + reason if reason else ""}')
        self.connected = False
        send_msg_all(f"{self.username} покинул чат")
        if self in CONNECTIONS_LIST:
            CONNECTIONS_LIST.remove(self)

    def send_msg(self, message): # отправка сообщения
        if self.connected:
            send_text(self.conn, message)

    def receive_msg(self): # принятие сообщения
...
def save_users(): # сохранение данных о пользователях в файл users.json
